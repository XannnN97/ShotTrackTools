...
from shottracktools_utils import parse_track, get_timeline_info, load_config, save_config
from lib import renamer, sequential, png_exporter, remove_suffix, edl_exporter
...
def execute(type, params):
    ...
    if type == "batch_renamer":
        ...
        return {"success": True, "logs": r["logs"], "undoData": r.get("undoData")}
    elif type == "edl_export":
        r = edl_exporter.run(resolve, params)
        return {"success": True, "logs": r}
    else:
...
def get_tracks():
    ...
    if not info:
        return {"success": False, "tracks": [], "timelineName": ""}
    return {"success": True, "tracks": info["tracks"], "timelineName": info["timelineName"]}
...
def main():
    ...
    if action == "execute":
        ...
        result = set_config(data.get("params", {}))
    else:
