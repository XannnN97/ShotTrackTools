const I18N = {
  zh: {
    ...
    png_export: "PNG/XML 导出", remove_suffix: "去后缀", edl_export: "EDL 导出",
    target_track: "目标轨道", search: "查找内容", replace: "替换为", prefix: "前缀",
    start_num: "起始编号", step: "步长", padding: "编号位数", output_dir: "输出目录",
    filename: "文件名", remove_suffix_label: "去掉文件后缀", execute: "执行", undo: "撤回",
    ...
    edl_desc: "导出目标轨道 EDL 文件", executing: "执行中...", executed: "执行完成",
    undone: "已撤回", error: "错误", select_func: "请选择左侧功能",
    timeline: "时间线", refresh: "刷新", no_timeline: "无时间线"
  },
  en: {
    title: "ShotTrackTools", version: "Version", batch_renamer: "Batch Replace", batch_sequential: "Sequential",
    png_export: "PNG/XML Export", remove_suffix: "Remove Suffix", edl_export: "EDL Export",
    target_track: "Target Track", search: "Search", replace: "Replace", prefix: "Prefix",
    start_num: "Start Number", step: "Step", padding: "Padding", output_dir: "Output Directory",
    filename: "Filename", remove_suffix_label: "Remove File Suffix", execute: "Execute", undo: "Undo",
    ...
    edl_desc: "Export target track EDL file", executing: "Executing...", executed: "Executed",
    undone: "Undone", error: "Error", select_func: "Please select a function from the left",
    timeline: "Timeline", refresh: "Refresh", no_timeline: "No Timeline"
  }
};
...
class App {
  ...
  updateLanguage() {
    ...
    document.getElementById('versionText').textContent = this.t('version') + ' 1.2.0';
  ...
  async saveConfig() {
    ...
    this.configCache[this.currentFunc] = params;
    try {
      await window.electronAPI.setConfig(this.configCache);
    } catch (e) {
  ...
  async startTrackPolling() {
    ...
    this.trackPollTimer = setInterval(() => this.refreshTracks(), 3000);
  }
  ...
  async refreshTracks() {
    try {
      ...
      if (result.success) {
        ...
        const newTimeline = result.timelineName || '';
        // 只在变化时更新
        if (JSON.stringify(newTracks) !== JSON.stringify(this.trackList) || newTimeline !== this.timelineName) {
          this.trackList = newTracks;
          this.timelineName = newTimeline;
          this.updateTrackDropdown();
          document.getElementById('timelineName').textContent = newTimeline || this.t('no_timeline');
        }
      }
    } catch (e) {
      console.error('Track polling error:', e);
    }
  }
  ...
  updateTrackDropdown() {
    ...
    select.innerHTML = '<option value="">-- ' + this.t('target_track') + ' --</option>' +
      this.trackList.map(t => '<option value="' + t + '">' + t + '</option>').join('');
    if (currentVal && this.trackList.includes(currentVal)) {
  ...
  selectFunction(funcKey) {
    ...
    this.renderSidebar();
    const cached = this.configCache[funcKey] || {};

    const trackField = `
      <div class="param-group">
        <label class="param-label">${this.t('target_track')}</label>
        <select class="param-input" id="trackSelect" style="width:100%;padding:8px 12px;">
          <option value="">-- ${this.t('target_track')} --</option>
          ${this.trackList.map(t => '<option value="' + t + '">' + t + '</option>').join('')}
        </select>
      </div>`;

    const forms = {
      batch_renamer: trackField + `
        <div class="param-group"><label class="param-label">${this.t('search')}</label><input type="text" class="param-input" id="p_search" value="${cached.search || 'sq1300'}"></div>
        <div class="param-group"><label class="param-label">${this.t('replace')}</label><input type="text" class="param-input" id="p_replace" value="${cached.replace || 'sq1400'}"></div>
        <div class="checkbox-row"><input type="checkbox" id="p_remove_suffix" ${cached.remove_suffix !== false ? 'checked' : ''}><label>${this.t('remove_suffix_label')}</label></div>`,
      batch_sequential: trackField + `
        <div class="param-group"><label class="param-label">${this.t('prefix')}</label><input type="text" class="param-input" id="p_prefix" value="${cached.prefix || '2Esq1400_'}"></div>
        <div class="param-group"><label class="param-label">${this.t('start_num')}</label><input type="text" class="param-input" id="p_start" value="${cached.start || 10}"></div>
        <div class="param-group"><label class="param-label">${this.t('step')}</label><input type="text" class="param-input" id="p_step" value="${cached.step || 10}"></div>
        <div class="param-group"><label class="param-label">${this.t('padding')}</label><input type="text" class="param-input" id="p_padding" value="${cached.padding || 4}"></div>
        <div class="checkbox-row"><input type="checkbox" id="p_remove_suffix" ${cached.remove_suffix !== false ? 'checked' : ''}><label>${this.t('remove_suffix_label')}</label></div>`,
      png_export: trackField + `
        <div class="param-group"><label class="param-label">${this.t('output_dir')}</label>
          <div class="output-row"><input type="text" class="param-input" id="p_output_dir" value="${cached.output_dir || ''}" placeholder="Desktop">
            <button class="btn btn-secondary" id="btnBrowse">${this.t('browse')}</button></div></div>
        <div class="checkbox-row"><input type="checkbox" id="p_remove_suffix" ${cached.remove_suffix !== false ? 'checked' : ''}><label>${this.t('remove_suffix_label')}</label></div>`,
      edl_export: trackField + `
        <div class="param-group"><label class="param-label">${this.t('output_dir')}</label>
          <div class="output-row"><input type="text" class="param-input" id="p_output_dir" value="${cached.output_dir || ''}" placeholder="Desktop">
            <button class="btn btn-secondary" id="btnBrowse">${this.t('browse')}</button></div></div>
        <div class="param-group"><label class="param-label">${this.t('filename')}</label><input type="text" class="param-input" id="p_filename" value="${cached.filename || 'shot_track'}"></div>`,
    ...
    // 恢复缓存的轨道选择
    if (cached.track && document.getElementById('trackSelect')) {
      document.getElementById('trackSelect').value = cached.track;
    }

    if (funcKey === 'png_export' || funcKey === 'edl_export') {
  ...
  getParams() {
    ...
    p.track = gt('trackSelect') || 'V1';
    if (this.currentFunc === 'batch_renamer') {
      p.search = gt('p_search'); p.replace = gt('p_replace'); p.remove_suffix = gc('p_remove_suffix');
    } else if (this.currentFunc === 'batch_sequential') {
      p.prefix = gt('p_prefix'); p.start = parseInt(gt('p_start')) || 10;
      ...
      p.output_dir = gt('p_output_dir'); p.remove_suffix = gc('p_remove_suffix');
    } else if (this.currentFunc === 'edl_export') {
      p.output_dir = gt('p_output_dir'); p.filename = gt('p_filename') || 'shot_track';
    }
    return p;
  }
